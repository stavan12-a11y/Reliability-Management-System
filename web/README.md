...
1. Copy `.env` and fill in real values:
   ...
   - `OPENAI_API_KEY` — only needed for the "AI history" tab (Phase 2 RAG lookup). Leave blank and the tab shows a plain "not configured" message instead of erroring. `OPENAI_CHAT_MODEL` optionally overrides the default `gpt-4o-mini`.
...
4. (Optional) Once `OPENAI_API_KEY` is set, backfill embeddings for the seeded history/maintenance records so the "AI history" tab has something to retrieve:
   ```bash
